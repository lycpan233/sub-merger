...
import { SubUserInfo } from "../types/types";
...
function updateTotalUserInfo(
  totalUserInfo: SubUserInfo,
  newUserInfo: SubUserInfo,
) {
...
async function getSubscribeYaml(subArr: any[], envConfig: any): Promise<any[]> {
  ...
  const promises = subArr.map(async (oneSub) => {
    try {
      ...
      if (response.ok) {
        const tmpSubUserInfo = parseSubscriptionUserInfo(
          response.headers.get("subscription-userinfo"),
        );
        updateTotalUserInfo(subUserInfo, tmpSubUserInfo);
        const yamlContent = await response.text();
        console.debug(
          `getSubscribeYaml: 获取订阅源信息成功，订阅源名称：${oneSub.subName}`,
        );
        return getProxyNodesFromYaml(yamlContent, oneSub.subName);
      } else {
        console.error(
          `getSubscribeYaml: 请求失败: Name: ${oneSub.subName}, URL: ${oneSub.subUrl}, 状态码: ${response.status}`,
        );
        sendBarkNotification(
          barkOpenid,
          "CF-Sub-Merger: 订阅源更新失败",
          `订阅源名称：${oneSub.subName}，HTTP响应状态码：${response.status}`,
        );
        return [];
      }
    } catch (error) {
      console.error(
        `getSubscribeYaml: 获取订阅内容时发生异常: Name: ${oneSub.subName}, URL: ${oneSub.subUrl}`,
        error,
      );
      sendBarkNotification(
        barkOpenid,
        "CF-Sub-Merger: 订阅源更新失败",
        `订阅源名称：${oneSub.subName}，错误信息：${error}`,
      );
      return [];
    }
  });
